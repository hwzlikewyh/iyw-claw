...
- [x] **Step 1: 固定 Widget dependencies**
...
- [x] **Step 2: 实现官方 App 生命周期**

```ts
import { App } from "@modelcontextprotocol/ext-apps"

const app = new App(
  { name: "Infinite Canvas", version: "0.1.11" },
...
- [x] **Step 4: 实现 display mode 和消息接口**
...
- [x] **Step 3: 实现 revision conflict 重放**
...
- [x] **Step 1: 实现 128 KiB chunk uploader**
...
开始前拒绝空文件；不设置插件级单文件总大小上限。取消时调用空 finalize/cancel 分支清理
临时文件。UI 显示累计字节，不按块刷 toast。

- [x] **Step 2: 实现按需读取和 object URL cache**
...
- [x] **Step 4: 验证 bridge 上限**

构造 128 KiB 随机 chunk，序列化完整 tools/call JSON；Expected: UTF-8 byte length < 256 KiB。
构造多个 128 KiB chunks；Expected: 每条 tools/call 的 UTF-8 长度低于 bridge 上限，持续上传不因插件总大小门禁被截断。
...
- [x] **Step 1: 生成 Vite metafile 和单 HTML**
