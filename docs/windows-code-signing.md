...
正常 Release 的 `release-windows-staged.yml` 强制要求硬件签名。初始化时检查
`IYW_CLAW_SAFENET_PIN` 和证书指纹，CI 不等待交互式登录。PIN 不重试、不写入命令行；
KSP 解锁完成仍必须通过真实签名探针，不能将“解锁返回成功”等同于签名可用。

签名机的 GitHub checkout、codeload 和资产下载使用现有本地代理；API 和上传域名
沿用原直连设置。Git 使用 HTTP/1.1。不要把全部 GitHub 下载域名放入 `NO_PROXY`：
v0.1.215 的实际失败为直连连接重置/超时，发生在签名前。JSON BOM 的解析修复
属于脚本解析层，不应据此强制下载连接绕过代理。

签名前输入通过 GitHub release asset ID 的二进制接口下载，避免再次按 tag 查找
草稿。使用 curl 显式连接现有代理，默认 `http://127.0.0.1:7890`，可由仓库变量
`WINDOWS_RELEASE_DOWNLOAD_PROXY` 指定 HTTP/HTTPS/SOCKS5 代理。下载子进程覆盖
`NO_PROXY`，资产跳转也经过代理；其他 API/上传的直连配置保持原样。

最多尝试三次，每次五分钟，重试间隔为五秒、十秒。连接超时 20 秒，连续 45 秒
低于 16 KiB/s 则停止当前尝试。未完成文件按不可变 asset ID 保存为 `.part`，
后续重试及重新运行从已有字节续传，不再每次整包重下；服务器不支持续传时
清理片段再从头下载。完整本地制品仍按大小和 SHA-256 复用，摘要核验通过后才
成为可解压输入。网络故障不会触发重新编译或私钥操作。

认证 header 和代理配置通过子进程标准输入传递，不写入命令行或日志；HTTPS
跳转到资产域名时不转发 GitHub Authorization。未配置公共镜像，避免草稿包凭证
流入第三方服务。日志记录代理/直连、断点字节、HTTP 状态、速度和失败退出码。

2026-09-18 在签名机所在 Windows 环境用同一本地代理下载 v0.1.216 的实际
待签名 ZIP（356638122 字节），耗时 29.97 秒、约 11.9 MB/s，大小及 GitHub
SHA-256 核验成功。此结果验证实际下载链路，不代表整轮硬件签名已通过。
同一资产的 Range 请求返回 HTTP 206；再次调用下载入口命中已验证本地缓存。

`sign-staged-windows.mjs` 将一次签名拆成两步：

1. 使用硬件证书签名，最多等待 180 秒；随后验证 Authenticode 信任链和指定证书。
2. 对已签名文件单独请求 RFC 3161 时间戳，每次最多 60 秒，最多三次。
   时间戳重试不再次调用私钥；最终必须同时通过信任链、预期证书和时间戳检查。

已由预期证书签名且带有效时间戳的文件直接复用。工具超时但文件已完成签名时，
只在校验通过后继续；硬件签名失败不盲目重试，以免重复触发令牌登录。
时间戳地址支持仓库变量 `WINDOWS_SIGN_TIMESTAMP_URL`，默认仍为 DigiCert。
所有修改均发生在 updater `.sig` 生成之前，时间戳缺失或校验失败仍阻止发布。

解锁子进程、探针、封装和最终验证均有独立超时，分别为 90 秒、12 分钟、30 分钟、
2 分钟。封装超时会输出已收集的诊断日志。整次签名 job 仍限制为 60 分钟。
以上是软件容错与诊断改进，不替代真实 CI 会话中的 USB 令牌和中间件验证。
