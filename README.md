...
重复安装先读取当前版本的 Fusion 环境计划，再检查本地文件与制品身份；匹配的组件
直接复用，不重新下载或解压。普通安装准备阶段不计算全量文件摘要，提交时统一校验
一次；显式修复和诊断仍进行完整检查。Agent Reach 中有对应受保护源码的 Python
字节码缓存允许正常生成或变化，不会因此误触发修复；源码与无源码字节码仍受校验。
