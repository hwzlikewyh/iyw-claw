//! `iyw-claw-server --supervise` — a minimal process supervisor that owns the
...
//! [`crate::update::runtime::EXIT_RESTART`]. We wait `IYW_CLAW_RESTART_DELAY_MS`
...
//! `IYW_CLAW_UPGRADE_TRIAL_SECS` (the new binary can't even boot), we restore
