...
fn load_and_run(symbol: &[u8]) -> Result<i32, String> {
    ...
    if symbol == WORKER_ENTRY {
        eprintln!("[internal-xinghe-worker] stage=load_library status=begin");
    }
    let path = resolve_library_path()?;
    unsafe {
        // The library remains live until its C ABI entry point returns.
        let library = load_library(&path).map_err(|error| library_error("load_library", error))?;
        identity::validate_loaded(&library)?;
        let abi = library
            .get::<unsafe extern "C" fn() -> u64>(ABI_ENTRY)
            .map_err(|_| "内置星河运行时缺少版本接口，请修复安装".to_string())?;
        let core_version = library
            .get::<unsafe extern "C" fn() -> u64>(CORE_VERSION_ENTRY)
        ...
        for required in [WORKER_ENTRY, HELPER_ENTRY] {
            library
                .get::<unsafe extern "C" fn() -> i32>(required)
        ...
        if symbol == WORKER_ENTRY {
            eprintln!(
                "[internal-xinghe-worker] stage=load_library status=ok elapsed_ms={}",
...
fn library_candidates() -> Vec<PathBuf> {
    ...
    if let Some(executable) = std::env::current_exe().ok() {
        if let Some(directory) = executable.parent() {
            ...
            candidates.push(
                directory
                    .join("xinghe-resources")
                    .join(worker_library_filename()),
            );
            candidates.push(
                directory
                    .join("../Resources/xinghe-resources")
                    .join(worker_library_filename()),
            );
            candidates.push(
                directory
