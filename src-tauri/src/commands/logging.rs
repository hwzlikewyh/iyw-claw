...
/// global level and per-target overrides, plus whether `IYW_CLAW_LOG`/`RUST_LOG`
...
pub async fn set_log_settings_core(
    ...
    app_metadata_service::upsert_value(conn, LOGGING_LEVEL_KEY, &serialized)
        .await
        .map_err(AppCommandError::from)?;
    // The env var (IYW_CLAW_LOG/RUST_LOG) owns the live level when set; persist the
    ...
    fn read_log_file_reads_full_and_tail() {
        ...
        temp_env::with_vars(
            ...
            || {
                ...
                std::fs::write(logs.join("iyw-claw.2026-01-01.log"), b"0123456789").unwrap();
                assert_eq!(
                    read_log_file_core("iyw-claw.2026-01-01.log", None).unwrap(),
                    "0123456789"
                );
                // Capped read returns the newest tail.
                assert_eq!(
                    read_log_file_core("iyw-claw.2026-01-01.log", Some(4)).unwrap(),
                    "6789"
                );
            },
        );
