...
//! `images` / `memory` / `memory-proposal`). A separate `--features platform`
//! launch turns the
...
fn parse_args() -> Result<Args, String> {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "iyw-claw-mcp --parent-connection-id <uuid> --socket-path <path> --token <secret> [--parent-pid <pid>] [--features delegation,feedback,ask,sessions,images,memory,memory-proposal,platform] [--working-dir <path>]"
                );
                std::process::exit(0);
            }
            "--capabilities" => {
