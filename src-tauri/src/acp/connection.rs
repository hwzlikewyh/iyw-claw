...
fn enforce_managed_model_options(agent_type: AgentType, options: &mut [SessionConfigOptionInfo]) {
    let model_ids = crate::acp::provider_overlay::managed_model_ids_for(agent_type);
    let default_model = crate::acp::provider_overlay::managed_default_model_for(agent_type);
        ...
        .filter(|option| option.id == "model" || option.category.as_deref() == Some("model"))
    {
        ...
        if !model_ids.contains(&select.current_value.as_str()) {
            select.current_value = default_model.to_string();
        }
        select.options = model_ids
            .iter()
...
async fn emit_session_config_options_values(
    ...
    enforce_managed_model_options(agent_type, &mut mapped);
...
mod tests {
    ...
    fn build_new_session_request_sets_claude_raw_meta() {
        let cwd = std::path::PathBuf::from("/tmp/iyw-claw");
    ...
    // iyw-claw-mcp off — the known-good payload. The connection-layer gate
    ...
    fn deferred_unwraps_codebuddy_mcp_tool_name() {
        ...
        let input = Some(
            r#"{"params":{"agent_type":"codex","task":"build"},"toolName":"mcp__iyw-claw-mcp__delegate_to_agent"}"#
                .to_string(),
        );
        assert_eq!(
            codebuddy_deferred_tool_name(AgentType::CodeBuddy, &input).as_deref(),
            Some("mcp__iyw-claw-mcp__delegate_to_agent")
        );
    }
    ...
    fn deferred_gates_on_codebuddy_and_shape() {
        let wrapped = Some(
            r#"{"params":{"task_id":"a"},"toolName":"mcp__iyw-claw-mcp__cancel_delegation"}"#
                .to_string(),
        );
        ...
        for raw in [
            r#"{"toolName":"mcp__iyw-claw-mcp__delegate_to_agent"}"#, // no params
    ...
    fn rewritten_title_persists_across_status_only_updates() {
        ...
        let deferred = Some(
            r#"{"params":{"agent_type":"codex","task":"x"},"toolName":"mcp__iyw-claw-mcp__delegate_to_agent"}"#
                .to_string(),
        );
        assert_eq!(
            resolve_rewritten_title(
