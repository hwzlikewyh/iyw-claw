...
//! agent's prompt as `[title](iyw-claw://session/<conversation_id>)`. The agent
...
pub struct SessionInfo {
    pub found: bool,
    /// The id that was queried (iyw-claw's internal conversation PK). Echoed so the
        ...
        Self {
            ...
            note: Some(format!(
                "No session matches id {session_id}. It may have been deleted, \
                 or never imported into iyw-claw."
            )),
            ..Default::default()
        }
    }
...
/// Listener-facing access to resolve a session by its iyw-claw conversation id. The
...
pub trait SessionInfoAccess: Send + Sync {
    /// Resolve `session_id` (iyw-claw's internal conversation PK) into a
...
/// independently — `iyw-claw-mcp` is injected when ANY feature is enabled, and each
