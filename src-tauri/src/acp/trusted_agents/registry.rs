...
pub static REGISTRY_TRUSTED_AGENTS: [TrustedAgentDefinition; 28] = [
    trusted_agent!(
        "agoragentic-acp", "Agoragentic", Npx, "agoragentic-mcp@1.3.0",
        "agoragentic-mcp", &["--acp"], &[], &[], Node, Some("18.0.0"), "1.3.0", ACP_ONLY
    ),
    trusted_agent!(
    ...
    trusted_agent!(
        "auggie", "Auggie CLI", Npx, "@augmentcode/auggie@0.35.0", "auggie", &["--acp"],
        &[("AUGMENT_DISABLE_AUTO_UPDATE", "1")], &[], Node, Some("20.0.0"), "0.35.0", ACP_ONLY
    ),
    trusted_agent!(
    ...
    trusted_agent!(
        "deepagents", "DeepAgents", Npx, "deepagents-acp@0.1.7", "deepagents-acp", &[], &[], &[],
        Node, None, "0.1.7", ACP_ONLY
    ),
    trusted_agent!(
        "devin", "Devin", ManagedBinary, "devin", "./bin\\devin.exe", &["acp"], &[], &[], Bundled,
        None, "3000.4.25", ACP_ONLY
    ),
    trusted_agent!(
        "dimcode", "DimCode", Npx, "dimcode@0.3.15", "dimcode", &["acp"], &[], &[], Node, None,
        "0.3.15", ACP_ONLY
    ),
    trusted_agent!(
        "dirac", "Dirac", Npx, "dirac-cli@0.4.36", "dirac", &["--acp"], &[], &[], Node,
        Some("22.13.0"), "0.4.36", ACP_ONLY
    ),
    trusted_agent!(
        "factory-droid", "Factory Droid", Npx, "droid@0.197.0", "droid",
        &["exec", "--output-format", "acp-daemon"],
        &[("DROID_DISABLE_AUTO_UPDATE", "true"), ("FACTORY_DROID_AUTO_UPDATE_ENABLED", "false")],
        &[], Node, Some("20.0.0"), "0.197.0", ACP_ONLY
    ),
    trusted_agent!(
        "fast-agent", "fast-agent", Uvx, "fast-agent-acp==0.10.1", "fast-agent-acp", &["-x"],
        &[("FAST_AGENT_MODEL", "codexplan")], &[], Python, Some("3.12"), "0.10.1", ACP_ONLY
    ),
    trusted_agent!(
        "github-copilot-cli", "GitHub Copilot", Npx, "@github/copilot@1.0.80", "copilot", &["--acp"],
        &[], &[], Node, None, "1.0.80", ACP_ONLY
    ),
    trusted_agent!(
        "glm-acp-agent", "GLM Agent", Npx, "glm-acp-agent@1.5.0", "glm-acp-agent", &[], &[], &[],
        Node, None, "1.5.0", ACP_ONLY
    ),
    trusted_agent!(
        "goose", "Goose", ManagedBinary, "goose", "./goose-package\\goose.exe", &["acp"], &[], &[],
        Bundled, None, "1.46.0", ACP_ONLY
    ),
    trusted_agent!(
        "harn", "Harn", ManagedBinary, "harn", "harn.exe", &["serve", "acp"], &[], &[], Bundled,
        None, "0.10.103", ACP_ONLY
    ),
    trusted_agent!(
        "junie", "Junie", ManagedBinary, "junie", "./junie/junie.exe", &["--acp=true"], &[], &[],
        Bundled, None, "2783.5.0", ACP_ONLY
    ),
    trusted_agent!(
    ...
    trusted_agent!(
        "nova", "Nova", Npx, "@compass-ai/nova@1.1.35", "nova", &["acp"], &[], &[], Node,
        Some("22.12.0"), "1.1.35", ACP_ONLY
    ),
    trusted_agent!(
    ...
    trusted_agent!(
        "qoder", "Qoder CLI", Npx, "@qoder-ai/qodercli@0.2.14", "qodercli", &["--acp"], &[], &[],
        Node, Some("20.0.0"), "0.2.14", ACP_ONLY
    ),
    trusted_agent!(
        "qwen-code", "Qwen Code", Npx, "@qwen-code/qwen-code@0.21.13", "qwen",
        &["--acp", "--experimental-skills"], &[], &[], Node, Some("22.0.0"), "0.21.13", ACP_ONLY
    ),
    trusted_agent!(
