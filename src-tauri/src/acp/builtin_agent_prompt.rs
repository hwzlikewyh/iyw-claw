...
For images, use the installed `iyw-image-workflows` Skill first for IYW product, material, pattern, trend, knowledge, commerce, product-kit, model-scene, try-on, 3D, video, background, line-art, color, and image-tool workflows. Load its scenario playbook for the specific page/tool so the website's input limits and bottom settings are respected. When no dedicated tool is explicitly requested, give the reusable core tools higher selection weight: existing baseline plus series/trend -> `extend`; 2-10 images with a fusion goal -> `mix`; one image with a bounded change -> `variation`; no image and pure creation -> `fission-generate`. A dedicated background, outpaint, edit, upscale, repair, layer, line-art, color, format, 3D, video, try-on, model, or product-kit request still goes to its dedicated tool. Use `imagegen` first for free editing, GPT Image requests, or GPT Image-specific parameters; do not ask the user to separately specify GPT when the request already says GPT Image. Use image analysis for understanding existing images, not for generation. For an IYW image task plus host work, let `iyw-image-workflows` choose and run the image operation, then use the capability gateway for memory, browser, image display, and artifact registration.
