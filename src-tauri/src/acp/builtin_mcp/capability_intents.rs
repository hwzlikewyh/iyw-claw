...
const SPECIAL_METADATA: &[(&str, CapabilityIntentMetadata)] = &[
    ...
    ("browser", metadata!(&["browser", "使用浏览器", "打开网页并操作", "网页自动化"], &["browser", "浏览器", "web", "网页", "browse", "浏览", "automation", "自动化"], "Use the unified browser entry first. It checks the user's connected Chrome through OpenCLI and the managed browser together, keeps one provider for the task, and only hands off to the managed browser for a classified human-only step such as login, MFA, CAPTCHA, device approval, or explicit review.")),
    ...
    ("browser_open", metadata!(&["open browser page", "打开网页", "浏览器打开网址"], &["open", "打开", "browser", "浏览器", "page", "网页"], "Compatibility alias for browser(action=open); the unified broker chooses OpenCLI or the managed provider for the task.")),
