...
const SPECIAL_METADATA: &[(&str, CapabilityIntentMetadata)] = &[
    ...
    ("browser", metadata!(&["browser", "使用浏览器", "打开网页并操作", "网页自动化"], &["browser", "浏览器", "web", "网页", "browse", "浏览", "automation", "自动化"], "Use the unified browser entry first. New tasks use the managed browser and must not launch the user's external browser. Use OpenCLI only when a prior browser result explicitly provides an opencli: tab ID.")),
    ("browser_list_tabs", metadata!(&["list browser tabs", "列出浏览器标签页", "查看打开的网页"], &["list", "列出", "browser", "浏览器", "tab", "标签页"], "Compatibility alias for browser(action=list_tabs); use the unified browser entry when possible because it checks OpenCLI and the managed browser together.")),
    ("browser_open", metadata!(&["open browser page", "打开网页", "浏览器打开网址"], &["open", "打开", "browser", "浏览器", "page", "网页"], "Compatibility alias for browser(action=open); new tasks open in the managed browser.")),
    ("browser_snapshot", metadata!(&["browser snapshot", "获取网页快照", "读取页面"], &["snapshot", "快照", "browser", "浏览器", "read", "读取"], "Compatibility alias for browser(action=snapshot); inspect the selected provider before an action.")),
    ("browser_read", metadata!(&["read browser page", "读取网页数据", "浏览器获取公开数据"], &["read", "读取", "browser", "浏览器", "web", "网页", "data", "数据"], "Compatibility alias for browser(action=read); use for dynamic or authenticated page data when direct sources are insufficient.")),
    ("browser_click", metadata!(&["click browser element", "点击网页元素", "浏览器点击"], &["click", "点击", "browser", "浏览器", "element", "元素"], "Compatibility alias for browser(action=click); use a fresh snapshot reference or precise locator.")),
    ("browser_fill", metadata!(&["fill browser field", "填写网页表单", "浏览器输入"], &["fill", "填写", "browser", "浏览器", "form", "表单"], "Compatibility alias for browser(action=fill); verify important values after writing.")),
    ("browser_press", metadata!(&["press browser key", "按网页键盘按键", "浏览器按键"], &["press", "按键", "browser", "浏览器", "keyboard", "键盘"], "Compatibility alias for browser(action=press); the broker keeps the selected provider pinned.")),
    ("browser_scroll", metadata!(&["scroll browser page", "滚动网页", "浏览器滚动"], &["scroll", "滚动", "browser", "浏览器", "page", "页面"], "Compatibility alias for browser(action=scroll); use only the selected provider's exact tab.")),
    ("browser_wait", metadata!(&["wait for browser page", "等待网页", "浏览器等待"], &["wait", "等待", "browser", "浏览器", "page", "页面"], "Compatibility alias for browser(action=wait); prefer a bounded selector or state condition.")),
    ("browser_screenshot", metadata!(&["browser screenshot", "网页截图", "浏览器截图"], &["screenshot", "截图", "browser", "浏览器", "image", "图片"], "Compatibility alias for browser(action=screenshot); use for visual verification or user presentation.")),
    ("browser_close_tab", metadata!(&["close browser tab", "关闭浏览器标签页", "关闭网页"], &["close", "关闭", "browser", "浏览器", "tab", "标签页"], "Compatibility alias for browser(action=close_tab); close only the exact selected tab.")),
    ("browser_command", metadata!(&["advanced browser command", "浏览器高级操作", "完整网页自动化"], &["advanced", "高级", "browser", "浏览器", "command", "操作", "automation", "自动化"], "Compatibility alias for browser(action=advanced); read the provider Skill before using advanced arguments.")),
    ("browser_request_user_action", metadata!(&["request browser user action", "请求用户操作浏览器", "让用户操作网页"], &["request", "请求", "user", "用户", "action", "操作", "browser", "浏览器"], "Compatibility alias for browser(action=request_user_action); use only for a genuinely human-only step.")),
    ("browser_present", metadata!(&["present browser window", "展示浏览器窗口", "展示网页成果", "打开界面给用户看"], &["present", "展示", "browser", "浏览器", "window", "窗口", "ui", "界面"], "Compatibility alias for browser(action=present); display the current page in iyw-claw while preserving the selected operation provider.")),
    ("browser_close_window", metadata!(&["close browser window", "关闭浏览器显示窗口", "收起浏览器窗口"], &["close", "关闭", "browser", "浏览器", "window", "窗口"], "Compatibility alias for browser(action=close_window); preserve the managed tab while closing only its detached window.")),
    ("present_task_files", metadata!(&["task artifacts", "任务成果", "提交成果", "查询成果", "修改成果", "删除成果", "交付最终文件"], &["present", "list", "update", "delete", "artifact", "成果", "交付"], "Register only final task deliverables, not their intermediate assets: deliver the completed PPT, not its embedded images. Also list/get artifacts in the current conversation or entire current workspace, and update/delete current-conversation records by ID without deleting files.")),
    ...
    ("read_memory_policy", metadata!(&["read memory policy", "读取记忆规则", "自我学习规范", "memory preflight"], &["read", "读取", "memory", "记忆", "policy", "规则", "learning", "学习"], "Use before the first direct memory operation in each turn to load the complete current host policy.")),
    ("search_session_history", metadata!(&["search conversation history", "搜索历史会话", "查以前做过的任务", "历史任务"], &["search", "搜索", "history", "历史", "conversation", "会话", "task", "任务"], "Use for bounded historical task lookup; use memory_recall for prior decisions, preferences, and reusable experience.")),
    ("append_user_memory", metadata!(&["remember user fact", "记住用户事实", "保存用户偏好", "长期记忆"], &["remember", "记住", "append", "保存", "memory", "记忆"], "Proactively retain clear user-stated durable facts and future preferences; no special remember phrase or repeated confirmation is required.")),
    ("propose_user_memory", metadata!(&["propose memory", "候选记忆", "记录用户纠正", "建议保存偏好"], &["propose", "候选", "memory", "记忆", "correction", "纠正"], "Retain uncertain reusable observations as searchable candidates; clear durable preferences use append.")),
    ...
    ("retire_user_memory", metadata!(&["forget outdated memory", "遗忘过期记忆", "设置记忆有效期"], &["forget", "expire", "retire", "memory", "遗忘", "过期", "记忆"], "Exclude a recalled obsolete entry or experience immediately or at its evidenced expiry; retain source history for audit.")),
