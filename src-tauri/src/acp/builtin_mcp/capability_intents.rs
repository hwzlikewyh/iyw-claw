...
const SPECIAL_METADATA: &[(&str, CapabilityIntentMetadata)] = &[
    ...
    ("browser_request_user_action", metadata!(&["request browser user action", "请求用户操作浏览器", "让用户操作网页"], &["request", "请求", "user", "用户", "action", "操作", "browser", "浏览器"], "Use when the Agent needs the user to complete a visible browser step such as login, MFA, CAPTCHA, or drag-and-drop.")),
