...
pub(super) fn tool() -> Value {
    json!({
        "name": FETCH_URL_TOOL,
        "description": "Call a known IYW website API using the current iyw-claw login. Provide description naming the current action; it is displayed as progress, never sent to the API. The iyw-capability-gateway Skill indexes business APIs by domain; load only the matching reference. Remaining business operations use this tool, while image generation/processing and upload use dedicated tools. Supports HTTPS iyw.cn and all subdomain levels, except disabled operations. Built-in image-to-3d and its ImageTo3D endpoint are disabled; never retry them through this tool, browser UI or another wrapper. Use endpoints and parameters from the user, official documentation, or observed requests. Supports GET, POST (default), PUT, PATCH, DELETE, HEAD, and OPTIONS; method names are case-insensitive. GET/HEAD cannot carry bodies. body_type selects json (default), form (URL-encoded fields), or text (raw string, including XML); override content-type in headers when needed. query and form fields accept scalars or scalar arrays (repeated keys), with null omitted. The host supplies the current token and browser defaults (Origin https://tu.iyw.cn, Referer https://tu.iyw.cn/trendPop?from=portal). Additional headers may override defaults but cannot replace credentials, Host, or transport headers. For www.iyw.cn/gateway/, /msgapi/, and /api/ the host also supplies the iyuanwu_token Cookie and portal Origin/Referer. Never supply tokens or cookies. Every tool execution returns the outputSchema envelope: ok, status, content_type, body_type, body, error. ok means HTTP 2xx and a complete response, not business success; inspect body business codes/messages. body_type is json, text, base64, or empty. Failures preserve HTTP status/body when available; status is null before any response. HTTP non-2xx and tool failures set the MCP error flag. The HTTP timeout defaults to 60 seconds and timeout_seconds accepts 1-900; encoded request and raw response bodies are limited to 2 MiB. Follow at most five redirects within the allowed HTTPS domains; rejected redirects return 3xx. No automatic retries: a timeout/cancellation or incomplete response may follow an executed operation. Only perform user-authorized operations and never blindly resubmit writes. Prefer dedicated tools when available, such as generate_iyw_image for image production and upload_iyw_file for arbitrary local files up to 1 GiB. No capability search/read is required.",
        "inputSchema": {
            "type": "object",
            "required": ["url", "description"],
            "properties": {
                "description": super::iyw_progress::schema(),
                "timeout_seconds": {"type": "integer", "minimum": 1, "maximum": request::MAX_TIMEOUT_SECONDS, "default": request::DEFAULT_TIMEOUT_SECONDS, "description": "Per-request HTTP timeout, in seconds. Default 60; use the documented budget for slow operations, e.g. 90 for product copy/planning or 600 for knowledge creation/deletion and batch archive downloads. Streaming responses are still buffered and the 2 MiB body limits remain. No automatic retry."},
                ...
                "headers": {"type": "object", "additionalProperties": {"type": "string"}, "description": "Optional HTTP headers. Authentication (including tokenInfo), cookies, Host, framing, connection, and proxy headers are reserved for the host."},
