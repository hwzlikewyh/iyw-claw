...
fn image_tool() -> Value {
    json!({
        "name": IMAGE_TOOL,
        "description": "Generate or edit IYW images directly. IYW platform image operations have highest routing priority: use fission for text-only creation, variation/extend/mix for source images, or the matching specialized platform operation. Only fall back to generate (images/generations) or edit (images/edits) after an explicit terminal platform failure or a confirmed rejection before task creation. Before that fallback, call list_iyw_image_models, select a model supporting the required operation, and pass its exact id in parameters.model. Reuse the catalog for the same task or batch. Platform operations need no Fusion model lookup. IMPORTANT: default timeouts are 600 seconds for IYW platform requests/polling and 300 seconds for generate/edit; the agent may override either with wait.timeoutSeconds, including values above 600. Prefer these defaults or longer for slow image tasks; do not choose a short timeout merely to return sooner. This tool has no image-generation capability_id; use its advertised definition, never read_iyw_capability or invoke_iyw_capability for image production. No capability search, separate upload, or extra image Skill is needed. Use single-task fields for one task, or requests for up to eight independent tasks. count intentionally starts multiple charged executions; never combine it with parameters.n or parameters.batchSize. A batch is fully validated before execution, continues after runtime item failures, and returns partial results in input order with successful URLs registered together. Ordinary image generation/editing delivers successful images directly using returned status, URLs, and delivery metadata. Inspect visuals when requested review, comparison, visual acceptance, or integration into a composed deliverable requires it. A detailed prompt alone does not require a separate quality-review call. Report partial or failed status honestly and do not regenerate beyond scope. A backend rejection does not mean the tool identity is wrong; preserve the returned error and do not invent a capability ID. A timeout, transport error, or non-terminal result is not a confirmed generation failure: query the original task_id when available; never blindly retry or switch to edit/generate after an uncertain submission.",
        "inputSchema": {
            "type": "object",
