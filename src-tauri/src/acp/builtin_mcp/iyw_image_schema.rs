...
pub(super) fn tool() -> Value {
    json!({
        ...
        "description": concat!(
            ...
            "For generate/edit first call list_iyw_image_models, choose a model supporting the operation, and copy its exact id into parameters.model. No default model or prior platform failure is required. Reuse the catalog for the same task or batch. ",
            ...
            "User-facing replies, progress and delivery descriptions use business display names only, or a generic image-processing name; never append internal model/provider IDs or backend names. Describe only execution confirmed by the result, using metadata.model_name when available. ",
