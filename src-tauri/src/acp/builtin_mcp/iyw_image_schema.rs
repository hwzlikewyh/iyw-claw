...
pub(super) fn tool() -> Value {
    json!({
        ...
        "description": concat!(
            ...
            "For generate/edit first call list_iyw_image_models, choose a model supporting the operation, and copy its opaque model_ref unchanged into parameters.model. No default model or prior platform failure is required. Reuse the catalog for the same task or batch. ",
            "Platform defaults are variation for one-image changes, mix for 2-10 references, and extend for four-panel grids or series from one base image. Keep references and describe the composition in prompt. With no base, use generate. Matching specialized operations and explicit fission remain available. Platform types need no Fusion model lookup, and passing a catalog model to them does not select that Fusion model. ",
