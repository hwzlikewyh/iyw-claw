...
pub(super) fn tool() -> Value {
    json!({
        ...
        "description": "List currently available Fusion image models and their types/capabilities before generate_iyw_image uses type=generate (images/generations) or type=edit (images/edits), including auto with no source images. Call with {}. Returns model IDs, display names, descriptions, generation/editing capabilities, and prices. Choose a model yourself from these results according to the user's task and model preference; require image_generation=true for generate or image_editing=true for edit, then copy its exact id to parameters.model. Do not guess model IDs or infer capabilities from names. Reuse this catalog for the same task or batch; refresh when availability changes. An empty catalog or failed lookup means no model can be selected from this response. This read-only tool does not generate images, upload inputs, or need capability search/read. Specialized IYW operations such as variation, extend, and mix do not use this catalog.",
        "inputSchema": {
            "type": "object",
            "properties": {},
