...
use crate::acp::delegation::transport::{
    ...
    client_memory_proposal_round_trip, client_platform_tools_call_round_trip,
    client_platform_tools_list_round_trip, client_round_trip, client_session_round_trip,
    ...
    BrokerMemoryAppendRequest, BrokerMemoryProposalRequest, BrokerPlatformToolsCallRequest,
    BrokerPlatformToolsListRequest, BrokerRequest, BrokerResponse, BrokerSessionRequest,
    BrokerStatusRequest,
};
...
/// `tools/list` response. The binary schedules its delivery only after stdout
/// has been flushed to the Agent.
...
/// stdin loop. The host-side wait is five seconds; this shorter hard bound
/// ensures the detached report task is reclaimed first.
pub async fn send_companion_ready_report(socket_path: String, report: BrokerCompanionReadyRequest) {
    ...
    {
        Ok(Ok(_)) => {}
        Ok(Err(error)) => tracing::warn!("companion ready report failed: {error}"),
...
mod tests {
    ...
    async fn tools_list_returns_three_delegation_tools() {
        ...
        // delegate_to_agent schema enumerates all 11 agent types.
        let delegate = tools
            .iter()
