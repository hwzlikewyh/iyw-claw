//! Self-cleanup watchdog for `iyw-claw-mcp`.
//!
...
//! (agent CLI that hangs, parent iyw-claw crash that orphans the agent) can
//! leave `iyw-claw-mcp` running forever, holding open the binary file and a
//! companion connection that no one will ever read from.
//!
//! When the parent iyw-claw / iyw-claw-server passes `--parent-pid <pid>` on
//! the command line, `iyw-claw-mcp` spawns this watchdog. It polls the OS
...
//! invisible next to the other work `iyw-claw-mcp` does.
//!
...
/// Default polling cadence. Fast enough that a stale `iyw-claw-mcp.exe`
...
/// permission glitch can't cause the watchdog to kill `iyw-claw-mcp` while
