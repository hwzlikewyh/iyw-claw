...
mod tests {
    ...
    fn test_parse_data_dir_arg_space_separated() {
        let args = vec![
            ...
            "--data-dir".to_string(),
            "/tmp/iyw-claw-data".to_string(),
        ];
        assert_eq!(
            parse_data_dir_arg(args),
            Some(std::path::PathBuf::from("/tmp/iyw-claw-data"))
        );
    }
    ...
    fn test_parse_data_dir_arg_equals_form() {
        let args = vec![
            "iyw-claw".to_string(),
            "--data-dir=/tmp/iyw-claw-data".to_string(),
            "--credential-helper".to_string(),
        ];
        assert_eq!(
            parse_data_dir_arg(args),
    ...
    fn test_parse_data_dir_arg_absent() {
        let args = vec!["iyw-claw".to_string(), "--credential-helper".to_string()];
    ...
    fn test_parse_data_dir_arg_missing_value_treated_as_absent() {
        ...
        let args = vec!["iyw-claw".to_string(), "--data-dir".to_string()];
        assert_eq!(parse_data_dir_arg(args), None);
    }
    ...
    fn test_helper_script_embeds_data_dir() {
        let tmp = std::env::temp_dir().join(format!("iyw-claw-cred-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&tmp).expect("create tmp dir");
        let binary = std::path::PathBuf::from("/usr/local/bin/iyw-claw-server");
        ...
        // deployments don't fall back to the hardcoded `app.iywclaw` path.
        // Paths are sh-single-quoted so spaces / `$` / backticks survive.
        assert!(content.contains("/usr/local/bin/iyw-claw-server"));
    ...
    fn test_helper_script_absolutizes_relative_data_dir() {
        // Regression: a relative `IYW_CLAW_DATA_DIR=data` previously produced
        ...
        let _guard = STATE_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let unique = format!("iyw-claw-rel-test-{}", uuid::Uuid::new_v4());
        ...
        let binary = std::path::PathBuf::from("./iyw-claw-server");
        ...
        assert!(
            !content.contains("'./iyw-claw-server'"),
    ...
    fn test_bat_double_quote_escapes_percent() {
        ...
        assert_eq!(
            bat_double_quote(r"C:\Users\%USERNAME%\iyw-claw"),
            "\"C:\\Users\\%%USERNAME%%\\iyw-claw\""
        );
    }
    ...
    fn test_helper_script_escapes_apostrophe_in_path() {
        ...
        let tmp = std::env::temp_dir().join(format!("iyw-claw-apos-test-{}", uuid::Uuid::new_v4()));
    ...
    fn test_credential_helper_e2e_server_mode() {
        ...
        let data_dir =
            std::env::temp_dir().join(format!("iyw-claw-helper-e2e-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&data_dir).expect("create data dir");

        // Save & override IYW_CLAW_DATA_DIR for the duration of this test —
        // server-mode `keyring_store` resolves `tokens.json` from this var.
        let saved_env = std::env::var("IYW_CLAW_DATA_DIR").ok();
        std::env::set_var("IYW_CLAW_DATA_DIR", &data_dir);
        ...
        rt.block_on(async {
            ...
            // Seed the file token store. Uses IYW_CLAW_DATA_DIR via
        ...
        match saved_env {
            Some(v) => std::env::set_var("IYW_CLAW_DATA_DIR", v),
            None => std::env::remove_var("IYW_CLAW_DATA_DIR"),
        }
