//! One-way import: Codex pets → iyw-claw pets.
//!
//! Codex stores pets at `${CODEX_HOME:-~/.codex}/pets/<id>/` with the same
//! `pet.json` + `spritesheet.webp` layout iyw-claw uses. We copy the directory
...
/// Copy selected Codex pets into `~/.iyw-claw/pets/`. Each entry copies the full
        ...
        let target_id = if pet.already_imported {
            if !request.overwrite_with_suffix {
                skipped.push(ImportSkipped {
                    ...
                    reason: "Pet with this id already exists in iyw-claw.".to_string(),
                });
                continue;
            }
