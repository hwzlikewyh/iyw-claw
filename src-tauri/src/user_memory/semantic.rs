    ...
    pub fn prepare_semantic_index(&self) -> Result<SemanticStatus, AppCommandError> {
        #[cfg(all(feature = "memory-semantic", target_pointer_width = "64"))]
        {
            ...
            tokio::spawn(async move {
                let result = service.prepare_semantic_inner().await;
...
#[cfg(all(feature = "memory-semantic", target_pointer_width = "64"))]
impl UserMemoryService {
    async fn prepare_semantic_inner(&self) -> Result<usize, AppCommandError> {
        ...
        self.prepare_managed_model(&crate::system_skills::data_dir_from_env(), &channel)
            .await?;
        self.synchronize_semantic().await
    }

    pub(super) fn schedule_semantic_refresh(&self) {
        if !self.semantic_status().model_installed {
            return;
        }
        let service = self.clone();
        tokio::spawn(async move {
    ...
    fn set_semantic_busy(&self, busy: bool) {
        let mut status = self
            .semantic
            .status
            .lock()
    ...
    fn mark_semantic_released(&self) {
        let mut status = self
            .semantic
            .status
            .lock()
    ...
    fn finish_semantic(&self, result: Result<usize, AppCommandError>) {
        let mut status = self
            .semantic
            .status
            .lock()
