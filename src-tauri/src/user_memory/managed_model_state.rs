...
pub(super) fn installed_version(data_dir: &Path) -> Option<String> {
    if cfg!(feature = "tauri-runtime") {
        current_directory(data_dir).ok()?;
        return crate::managed_environment::component_version(
            super::managed_model_types::MODEL_COMPONENT_ID,
        );
    }
    let root = managed_root(data_dir);
...
pub(super) fn installed_quick(data_dir: &Path) -> bool {
    if cfg!(feature = "tauri-runtime") {
        return desktop_directory().is_ok_and(|directory| validate_layout(&directory).is_ok());
    }
    let root = managed_root(data_dir);
    let pointer = super::structured_file::read_json_optional::<CurrentModelPointer>(
        &root,
...
fn desktop_directory() -> Result<PathBuf, AppCommandError> {
    crate::managed_environment::component_root(super::managed_model_types::MODEL_COMPONENT_ID)
        .ok_or_else(|| {
            AppCommandError::dependency_missing(
                "BGE 模型未安装或环境未就绪，请运行独立环境修复程序",
            )
        })
}
...
fn validate_version(version: &str) -> Result<(), AppCommandError> {
    let valid = version.len() <= 64 && semver::Version::parse(version).is_ok();
    valid
        .then_some(())
