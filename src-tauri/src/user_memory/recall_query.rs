...
pub(super) async fn collect_exact<C: ConnectionTrait>(
    ...
    let sql = format!(
        "SELECT id FROM memory_item_current WHERE id = ? AND {scope} AND trust_class IN ('host_confirmed', 'agent_experience', 'candidate') AND sensitive = 0 AND superseded_by IS NULL{validity} LIMIT ?"
    );
