...
use iyw_claw_lib::db::test_helpers::fresh_disk_db;
...
async fn build_server() -> (TestServer, tempfile::TempDir, tempfile::TempDir) {
    ...
    iyw_claw_lib::db::service::folder_service::add_folder(&db.conn, "/tmp/proj")
        .await
        .expect("seed folder");
...
async fn export_download_upload_inspect_stage_roundtrip() {
    ...
    let form = MultipartForm::new().add_part(
        "file",
        Part::bytes(bytes).file_name("iyw-claw-backup.iyw-claw.zip"),
    );
    let up = server
    ...
    assert!(data_dir
        .path()
        .join(".iyw-claw-restore-pending.json")
        .is_file());
}
