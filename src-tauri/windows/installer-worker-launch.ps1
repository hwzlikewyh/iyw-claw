param(
    ...
    [long]$OriginalBar, [long]$ProgressBar, [long]$StatusText, [long]$StageText
)
$ErrorActionPreference = 'Stop'
$arguments = '-NoProfile -NonInteractive -WindowStyle Hidden -ExecutionPolicy Bypass -File "' + $Directory + '\installer-worker.ps1" -Directory "' + $Directory + '" -AppVersion ' + $AppVersion + ' -InstallerPid ' + $InstallerPid + ' -OriginalBar ' + $OriginalBar + ' -ProgressBar ' + $ProgressBar + ' -StatusText ' + $StatusText + ' -StageText ' + $StageText
