...
function verify() {
  ...
  assert.equal(snapshot.components.length, 8, "Published acceptance binding must enable all eight managed tools")
  assert.equal(JSON.parse(run(["diagnose"])).status, "healthy")
  check("fresh eight-component install and native health probes")
