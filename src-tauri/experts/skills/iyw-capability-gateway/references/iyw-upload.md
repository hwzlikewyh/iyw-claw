...
| 字段 | 必填 | 含义 |
| --- | --- | --- |
| `path` | 是 | 当前 MCP 主机工作区内的普通文件，填真实文件系统路径；绝对路径或相对工作区路径 |
...
| 错误 | 下一步 |
| --- | --- |
| `invalid_request` / `not_started` | 根据提示修正参数。确认 `path`、`description` 都是字符串；文件确实在 MCP 主机工作区、可读，且路径未编码、未额外加引号。核实原因后更正一次，不轮流猜路径 |
...
若目标只是交付文件，可保留工作区内的完整文件，并通过 `present_task_files` 提交本地最终产物；这不等于云端上传成功。若下游接口必须使用公网 URL，应明确说明当前步骤受阻，待登录、网络或服务恢复后再继续，不能伪造 URL 或改用未经确认的上传服务。
