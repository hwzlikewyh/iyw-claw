...
- "Where should I create this skill? If you do not have a preference, I will place it in `~/.iyw-claw/skills` so iyw-claw can publish it to the enabled Agents."
...
Before running `init_skill.py`, ask where the user wants the skill created. If they do not specify a location, default to the current user's `~/.iyw-claw/skills` directory so iyw-claw can publish it to every installed and enabled Agent. When iyw-claw is running, its central-store watcher publishes the new Skill automatically; the script still accepts any explicit `--path` supplied by the user.
...
```bash
scripts/init_skill.py my-skill --path "${HOME}/.iyw-claw/skills"
scripts/init_skill.py my-skill --path "${HOME}/.iyw-claw/skills" --resources scripts,references
