...
优先使用 uv 在 skill 目录内创建独立环境。开始任何项目之前，先在 PowerShell 中
设置入口并同步锁定依赖：

```powershell
$skillDir = Join-Path $env:USERPROFILE ".iyw-claw\skills\bggg-creator-image2psd"
$image2psdCli = Join-Path $skillDir "scripts\image2psd.py"
$initProjectCli = Join-Path $skillDir "scripts\init_project.py"
uv sync --project $skillDir --python 3.13
```

后续命令统一使用 `uv run --project $skillDir --python 3.13 python ...`。`uv run`
会自动校验 `uv.lock` 并同步 `.venv`。只有 uv 不可用时，才使用已安装
`scripts/requirements.txt` 全部依赖的 Python 3.10 及以上环境直接运行脚本。
...
2. 创建 `projects/YYYYMMDD_slug/`，把源图和所有输出放进去。默认用脚本初始化：
   ...
   ```powershell
   uv run --project $skillDir --python 3.13 python $initProjectCli `
     lifestyle_product --source "用户附件.png" --date 20260503
   ```
...
```powershell
uv run --project $skillDir --python 3.13 python $image2psdCli assemble `
...
```powershell
uv run --project $skillDir --python 3.13 python $image2psdCli split-colors `
  "poster.png" `
