...
优先使用 uv 创建 skill 自己的 Python 3.13 环境并安装锁定依赖：
...
后续命令使用 `uv run --project "$skill_dir" --python 3.13`，它会自动校验
`uv.lock` 并同步环境。只有 uv 不可用时，才执行
`python3 -m pip install -r "$skill_dir/scripts/requirements.txt"` 并使用
Python 3.10 及以上版本。
