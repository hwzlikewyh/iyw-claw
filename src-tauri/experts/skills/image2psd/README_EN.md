...
Prefer uv to create a skill-local Python 3.13 environment and install the
locked dependencies:

```bash
skill_dir="$HOME/.codex/skills/bggg-creator-image2psd"
uv sync --project "$skill_dir" --python 3.13
```

Run subsequent commands through
`uv run --project "$skill_dir" --python 3.13`; it validates `uv.lock` and
synchronizes the environment automatically. Only when uv is unavailable, run
`python3 -m pip install -r "$skill_dir/scripts/requirements.txt"` and use
Python 3.10 or later directly.
...
The examples below use the `skill_dir` variable defined during installation.

Initialize a project folder:

```bash
uv run --project "$skill_dir" --python 3.13 python "$skill_dir/scripts/init_project.py" lifestyle_product \
  --source input.png
```
...
```bash
uv run --project "$skill_dir" --python 3.13 python "$skill_dir/scripts/image2psd.py" assemble \
  --manifest "$skill_dir/projects/20260503_lifestyle_product/manifest.json"
```

Assemble positional images directly:

```bash
uv run --project "$skill_dir" --python 3.13 python "$skill_dir/scripts/image2psd.py" assemble bg.png logo.png title.png \
  --first-is-background \
...
```bash
uv run --project "$skill_dir" --python 3.13 python "$skill_dir/scripts/image2psd.py" split-colors poster.png \
