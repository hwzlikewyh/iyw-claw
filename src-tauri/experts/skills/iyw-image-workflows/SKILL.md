...
description: 通过内置 Python CLI 独立检索 IYW 知识库、图片/报告/趋势/IP 资料，或调用已经确认的 IYW 图片工具接口，支持知识查询、分身生图、本地图片上传并自动违规检测、网络图片违规检测、变款、系列延伸、多图融合、编辑、扩图、高清修复、图案与线稿处理、格式转换、3D、视频和模特场景。用户提到 IYW 知识库、内部规范、品牌或 IP 手册、知识检索、商品图、上传检测、使用智能搜索获取趋势或主题素材并设计，或其他 IYW 产品、素材与商业图片任务时使用。自由创作或自由编辑优先使用 imagegen；用户明确要求 GPT Image 或 GPT Image 专用参数时也使用 imagegen，无需再次要求用户指定 GPT。
routing:
  capability: IYW 图片素材知识首选工作流
  coreTriggers: [IYW 知识资料或图片生成编辑上传审查]
  exclusions: [只需理解已有图片, 自由创作或自由编辑, 用户明确要求 GPT Image, 无需 IYW 数据接口的普通文本]
  aliases: [IYW 图片, IYW 知识库]
  invocation: IYW 产品素材知识与商业链路优先；自由创作编辑或 GPT Image 请求交给 imagegen。
---
...
在 IYW 产品、商品、素材、趋势、知识库、上传审查及商业图片工作流中，本 Skill
是默认首选。没有 IYW 业务链路要求的自由创作或自由编辑优先使用 `imagegen`；
用户明确要求 GPT Image 或 GPT Image 专用参数时也直接使用 `imagegen`，不要再次
询问是否指定 GPT。只理解、识别、比较或判断已有图片时，使用宿主提供的图片分析
路径，不触发图片生产工作流。
...
- 产生最终用户文件或公开 HTTPS URL 时，必须在最终回复前优先调用当前工具列表中实际可见的
