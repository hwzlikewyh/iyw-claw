...
routing:
  capability: 企业微信联系人文档日程消息等办公操作
  coreTriggers: [企业微信办公数据或链接操作]
  exclusions: [非企业微信平台的通用办公建议]
  aliases: [WeCom, 企业微信套件]
  invocation: 先读 SKILL.md，按业务域使用文档规定的 CLI 命令。
---
...
如果命令不存在、报错或版本号小于 `1.1.0`，停止业务操作，并告知用户：
“应用托管的企业微信 CLI 运行时不可用，请重启应用后重试”。不要自行执行全局 npm
安装，也不要改用用户环境中的其他 `wecom-cli`。
