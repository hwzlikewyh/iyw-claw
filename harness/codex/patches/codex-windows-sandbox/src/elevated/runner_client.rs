...
pub(crate) fn spawn_runner_transport(
    ...
    let runner_exe = crate::resolve_exe_for_launch(&std::env::current_exe()?, codex_home);
    #[cfg(feature = "bundled-host")]
    anyhow::ensure!(!registered_core_requested(), "registered Codex Core is not the embedded Xinghe host");
    ...
    let runner_cmdline = registered_alias
        ...
        .to_str()
        .map(str::to_owned)
        .unwrap_or_else(|| "xinghe-command-runner.exe".to_string());
    let runner_full_cmd = format!(
        "{} {} {} {}",
        quote_windows_arg(&runner_cmdline),
        if cfg!(feature = "bundled-host") { crate::embedded_roles::RUNNER_FLAG } else { "" },
