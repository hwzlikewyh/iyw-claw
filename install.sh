...
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/xintaofei/iyw-claw/main/install.sh | bash
#   curl -fsSL https://raw.githubusercontent.com/xintaofei/iyw-claw/main/install.sh | bash -s -- --version v0.5.0
#

set -euo pipefail

REPO="xintaofei/iyw-claw"
INSTALL_DIR="${IYW_CLAW_INSTALL_DIR:-/usr/local/bin}"
WEB_DIR="${IYW_CLAW_WEB_DIR:-/usr/local/share/iyw-claw/web}"
VERSION=""
# Stale iyw-claw-server / iyw-claw-mcp binaries elsewhere in PATH are removed by
# default so the user's `iyw-claw-server` command always runs the freshly
# installed binary AND the runtime locates the matching companion via the
# exe-sibling lookup. Set IYW_CLAW_NO_CLEANUP=1 (or pass --no-cleanup) to
# disable.
CLEANUP_CONFLICTS=1
...
# Names of binaries this installer manages. `iyw-claw-server` is the user-facing
# entry point; `iyw-claw-mcp` is the stdio MCP companion that the server's ACP
# layer spawns per session for delegation. Both must live in the same
# directory — `locate_iyw_claw_mcp_binary()` in src-tauri/src/acp/connection.rs
# resolves the companion as a sibling of the running server executable.
MANAGED_BINS=(iyw-claw-server iyw-claw-mcp)
...
DEST_BIN="${INSTALL_DIR}/iyw-claw-server"
DEST_BIN_REAL="$(canon_path "$DEST_BIN")"
INSTALL_DIR_REAL="$(canon_path "$INSTALL_DIR")"

# Scan PATH for both managed binaries — a stale `iyw-claw-mcp` in an earlier
# PATH entry would be picked by the runtime's `which` fallback once
# `iyw-claw-server` was upgraded out from under it, breaking delegation in
...
#
# Stop iyw-claw-mcp too: on Unix `cp` over a running binary succeeds (the
...
RESTARTED_PIDS=""
if pgrep -x iyw-claw-server >/dev/null 2>&1; then
  echo "Stopping running iyw-claw-server process(es)..."
  RESTARTED_PIDS=$(pgrep -x iyw-claw-server || true)
  if kill $RESTARTED_PIDS 2>/dev/null; then
    ...
    if pgrep -x iyw-claw-server >/dev/null 2>&1; then
      echo "Force stopping iyw-claw-server..."
...
if ! resolve_priv "$INSTALL_DIR"; then
  ...
  echo "       Re-run as root, install sudo, or set IYW_CLAW_INSTALL_DIR/IYW_CLAW_WEB_DIR to writable"
  echo "       paths (e.g. \$HOME/.local/bin and \$HOME/.local/share/iyw-claw/web)."
  exit 1
fi
if [ -n "$PRIV" ]; then
...
if [ -d "$WEB_SRC" ]; then
  ...
  if ! resolve_priv "$WEB_DIR"; then
    ...
    echo "       Re-run as root, install sudo, or set IYW_CLAW_WEB_DIR to a writable path."
    exit 1
  fi
...
# Final smoke: iyw-claw-mcp must exist next to iyw-claw-server so the runtime's
