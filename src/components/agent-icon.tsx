...
import { getAgentColor, getAgentInitial } from "@/lib/custom-agents"
import {
  isCustomAgentType,
  type AgentType,
...
function DeepSeekIcon({ size = "1em" }: AgentGlyphProps) {
  return (
    <svg
...
const AGENT_ICONS: Partial<
  Record<BuiltinAgentType, ComponentType<AgentGlyphProps>>
> = {
...
export function AgentIcon({ agentType, className }: AgentIconProps) {
  const Icon = isCustomAgentType(agentType) ? undefined : AGENT_ICONS[agentType]
  if (!Icon) {
    return (
      <span
        aria-hidden
