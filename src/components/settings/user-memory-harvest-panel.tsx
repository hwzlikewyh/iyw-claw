...
export function UserMemoryHarvestPanel({
  ...
  const runRescan = useCallback(async () => {
    ...
    const rescan = (module as RescanApi).rescanUserMemoryHarvest
    ...
    try {
      ...
      if (
        window.confirm(
          t("diagnostics.rescanPreview", {
            reQueued: result.preview.reQueued,
            retainedTerminal: result.preview.retainedTerminal,
          })
        )
      ) {
        await rescan(true)
  ...
  const runRebuildIndex = useCallback(async () => {
    const module = await import("@/lib/api")
    const rebuild = (module as RescanApi).rebuildUserMemoryCandidateIndex
    if (typeof rebuild !== "function") return
    setLoading(true)
    try {
      const preview = await rebuild(false)
      if (window.confirm(t("diagnostics.rebuildPreview", { affected: preview.affected }))) {
        await rebuild(true)
        await refresh()
      ...
      {harvest ? (
        <>
          <p className="flex flex-wrap gap-x-3 gap-y-1">
            <span>{t("diagnostics.harvestQueued", { queued: harvest.queued })}</span>
            <span>{t("diagnostics.harvestExtracting", { extracting: harvest.extracting })}</span>
            <span>{t("diagnostics.harvestProposed", { proposed: harvest.proposed })}</span>
            <span>{t("diagnostics.harvestNoop", { noop: harvest.noop })}</span>
            <span>{t("diagnostics.harvestFailed", { failed: harvest.failed })}</span>
            <span>{t("diagnostics.harvestDead", { dead: harvest.dead })}</span>
            <span>{t("diagnostics.harvestBacklog", { backlog: harvest.backlog })}</span>
          </p>
          <p className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-muted-foreground">
            {harvest.lastHarvestAt && (
              <span>
                {t("diagnostics.harvestLast", { time: formatTime(harvest.lastHarvestAt) })}
              </span>
            )}
