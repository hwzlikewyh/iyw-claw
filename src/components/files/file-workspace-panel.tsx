...
export function FileWorkspacePanel({ isVisible }: { isVisible: boolean }) {
  ...
  if (!isVisible && activeFileTab?.kind === "file") return null
  ...
  // Image preview
  const binaryKind = isFileTab && binaryPreviewKind(activeFileTab?.path)
  if (binaryKind && activeFileTab && activeIo) {
    return (
      <BinaryFilePreview
        key={`${activeFileTab.id}:${activeFileTab.mtimeMs ?? 0}`}
        rootPath={previewRoot ?? activeIo.rootPath}
        path={activeFileTab.path ?? activeIo.ioPath}
        kind={binaryKind}
      />
    )
  }
