...
const AGENT_URI = /^iyw-claw:\/\/agent\/(.+)$/i
const SESSION_URI = /^iyw-claw:\/\/session\/(.+)$/i
const COMMIT_URI = /^iyw-claw:\/\/commit\/.*@(.+)$/i
...
const SKILL_URI = /^iyw-claw:\/\/skill\/(.+)$/i
...
// message-input's `embeddedPayloadsRef`). `iyw-claw://` (not `file://`) is used on
...
const EMBEDDED_URI_PREFIX = "iyw-claw://embedded/"
...
/**
 * Parse a composer reference uri (`file://` / `iyw-claw://…`) back into
 ...
 */
export function parseIywClawReferenceUri(
  uri: string,
  label: string
): ReferenceAttrs | null {
  ...
  if (session) {
    const id = session[1]
    // Current format is `iyw-claw://session/<conversation_id>` (a bare numeric id),
    ...
    // LEGACY links `iyw-claw://session/<agent_type>_<external_id>` (still present in
