...
export interface RichComposerHandle {
  /** Serialize the current document to plain text with reference tokens. */
  getText: () => string
...
export interface RichComposerProps {
   ...
   * English fallbacks apply when omitted. Render-only.
   */
...
/**
 * Plain-text composer: a Tiptap editor with inline reference badges, IME-safe
...
export const RichComposer = forwardRef<RichComposerHandle, RichComposerProps>(
  function RichComposer(
    ...
    const editor = useEditor({
      ...
      editorProps: {
        attributes: {
          class: "iyw-claw-composer-content",
          role: "textbox",
          "aria-multiline": "true",
      ...
      onCreate: ({ editor }) => {
        ...
        // queue-edit document that a plain-text default cannot represent.
        onReadyRef.current?.()
      },
      onDestroy: () => {
    ...
    return (
      <div
        className={cn("iyw-claw-composer flex min-h-0 flex-col", className)}
        style={style}
        data-disabled={disabled || undefined}
      >
        <EditorContent
          editor={editor}
          className="iyw-claw-composer-scroll min-h-0 flex-1 overflow-y-auto px-3 py-2 text-base md:text-sm"
        />
