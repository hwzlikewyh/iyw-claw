...
describe("buildReferenceGroups", () => {
  ...
  it("adapts sessions into iyw-claw://session references", () => {
    const groups = buildReferenceGroups(
      "login",
      emptySources({
        sessions: [
    ...
    expect(sessions[0].reference.uri).toBe("iyw-claw://session/7")
  })
  ...
  it("adapts commits and filters by hash / message / author", () => {
    ...
    expect(commits[0].reference.uri).toBe("iyw-claw://commit/%2Frepo@abc12340000")
  })
