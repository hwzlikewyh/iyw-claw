...
/**
 * Re-derive Streamdown's default rehype pipeline so the app-internal `iyw-claw`
 ...
 * `iyw-claw`, so it strips the href off our `[label](iyw-claw://…)` reference links;
 ...
 * maps `<a>` to `MarkdownLink` (which turns a `iyw-claw:` href into an inline
 * badge). The net effect was `@Codex CLI [blocked]` in the transcript.
 *
 * Adding `iyw-claw` to the sanitize allow-list lets the href survive. harden is
 ...
 */
const APP_REFERENCE_PROTOCOLS = ["iyw-claw"]

export function rehypePluginsAllowingIywClaw(
  ...
  return Object.entries(defaults).map<RehypePlugin>(([key, plugin]) => {
    ...
    const next: SanitizeSchema = {
      ...
      protocols: {
        ...
        href: [
          ...href,
          ...APP_REFERENCE_PROTOCOLS.filter(
            (protocol) => !href.includes(protocol)
          ),
        ],
      },
    }
