...
export function isAgentLikeToolName(toolName: string): boolean {
  ...
  if (
    ...
    name === "create_goal" ||
    name === "update_goal" ||
    // iyw-claw-mcp ask_user_question — owns the AskQuestionResultCard, so it must
    ...
    // iyw-claw-mcp check_user_feedback — owns the FeedbackCheckResultCard capsule,
  ...
  if (/[^a-z0-9]show_interactive_html$/.test(name)) return true
