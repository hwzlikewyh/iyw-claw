...
describe("formatConversationTitle", () => {
  ...
  it("reduces session/commit/agent links (iyw-claw:// uris) to their label", () => {
    expect(
      formatConversationTitle("[My chat](iyw-claw://session/codex_abc)")
    ).toBe("My chat")
    expect(
      formatConversationTitle("[abc1234](iyw-claw://commit/%2Frepo@abc)")
    ).toBe("abc1234")
    // An agent reference keeps the `@` — it lives inside the bracket text.
    expect(formatConversationTitle("[@Codex](iyw-claw://agent/codex)")).toBe(
      "@Codex"
    )
  })
