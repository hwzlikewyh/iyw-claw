...
const EXACT_TOOL_NAME_ALIASES: Record<string, string> = {
  ...
  // iyw-claw multi-agent delegation MCP tools (server prefix varies by host)
  delegate_to_agent: "delegate_to_agent",
  "mcp__iyw-claw-mcp__delegate_to_agent": "delegate_to_agent",
  "mcp__iyw-claw-delegate__delegate_to_agent": "delegate_to_agent",
  "mcp__iyw-claw__delegate_to_agent": "delegate_to_agent",
  get_delegation_status: "get_delegation_status",
  cancel_delegation: "cancel_delegation",
  // iyw-claw-mcp live-feedback poll (server prefix varies by host; the suffix rule
  // in `normalizeToolName` covers the other separators). Codex persists it under
  // the bare `check_user_feedback` name, dropping the `mcp__iyw_claw_mcp` namespace.
  ...
  // iyw-claw-mcp ask-user-question companion tool (server prefix varies by host;
  // the suffix rule in `normalizeToolName` covers the other separators)
  "mcp__iyw-claw-mcp__ask_user_question": "question",
  lsp_diagnostics: "lsp",
...
function inferFromInput(
  ...
  // (plural) is the iyw-claw-mcp `ask_user_question` payload shape, so the live
...
export function normalizeToolName(toolName: string): string {
  ...
  // iyw-claw-mcp ask-user-question companion tool. Same host-prefix story as the
...
// Canonical names of the iyw-claw-mcp delegation companion tools. Each has a
...
export function inferLiveToolName(params: {
  ...
  // The iyw-claw-mcp delegation companion tools carry their authoritative identity
